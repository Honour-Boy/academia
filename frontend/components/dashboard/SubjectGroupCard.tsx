...
/**
 * Compact subject card for the teacher dashboard. Aggregates every class the
 * teacher teaches this subject in. Clicking drills into /subjects/[id] which
 * lists the class breakdown.
 */
export default function SubjectGroupCard({
  subjectId, subjectName, classCount, totalStudents, gradedStudents,
  term, academicYear,
}: Props) {
  const pct = totalStudents > 0 ? Math.round((gradedStudents / totalStudents) * 100) : 0
  const isComplete = totalStudents > 0 && gradedStudents >= totalStudents

  return (
    <Link
      href={`/subjects/${subjectId}?term=${encodeURIComponent(term)}&year=${encodeURIComponent(academicYear)}`}
